...
/*   Updated: 2026/03/02 15:37:48 by sgadinga         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
typedef struct s_reduce_op
{
	...
	void					(*apply)(void *acc, const void *value,
							t_dtype dtype);
 ...
 **
 ** A view is a tensor whose data belongs or points to the data of another tensor.
 ...
 ** @param l         Pointer to a tensor layout (describes it's ndim, shape,
 and stride).
 ** @param data      Pointer to some data (typically the parent's base or offset from it).
 **
 ...
 ** @return          Pointer to tensor view.
 */
t_tensr						*tensr_permute(const t_tensr *t,
								const size_t *perm);
 ...
 */
size_t						tensr_offset(const t_layout *l,
								const size_t *indices);
 ...
 ** with support for non-contiguous tensors.
 **
