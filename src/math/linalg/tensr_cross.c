		...
		const size_t *a_indices, const size_t *b_indices)
{
	...
	a_prod = mul_func(tensr_get(a, a_indices), tensr_get(b, b_indices), a->dtype);
	b_prod = mul_func(tensr_get(a, b_indices), tensr_get(b, a_indices), b->dtype);
	return (sub_func(a_prod, b_prod, a->dtype));
}
