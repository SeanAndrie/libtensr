...
t_tensr *tensr_elementwise(const t_tensr *a, const t_tensr *b, void (*f)(void *a, void *b, void *out, t_dtype dtype))
{
    t_iter  it;
    t_tensr *out;
    ...
    while (iter_next(&it))
    {
        ...
        f(tensr_get(a, a_indices), tensr_get(b, b_indices), tensr_get(out, it.indices), a->dtype);
    }
    return (out);
}
